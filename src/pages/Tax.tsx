...
export default function Tax() {
  ...
  const [tab, setTab] = useState<
    "summary" | "income" | "deductions" | "simulator"
  >("summary");

  const records = useLiveQuery(() => db.taxRecords.orderBy("year").toArray());
  const salaryRecords = useLiveQuery(() =>
    db.salaryRecords.orderBy("year").toArray(),
  );
  const current = useLiveQuery(
    () => db.taxRecords.where("year").equals(year).first(),
    [year],
  );
  ...
  useEffect(() => {
    if (!records) return;
    if (records.some((r) => r.year === year)) return;
    ...
    if (salRec) {
      ...
      empty.pvdContribution =
        salRec.baseSalary * 12 * (salRec.pvdEmployeeRate / 100);
    }
    db.taxRecords.add(empty);
  }, [records, year, salaryRecords]);
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
      ...
      <div className="bg-white flex items-center gap-3 px-4 py-4 border-b border-gray-100">
        ...
        <div className="flex-1 text-[17px] font-bold text-gray-900">
          วางแผนภาษี
        </div>
        <select
          value={year}
      ...
      <div className="flex-1 overflow-y-auto">
        <div className={tab === "summary" ? "" : "hidden"}>
          <SummaryTab year={year} onSwitch={setTab} />
        </div>
        {tab === "income" && <IncomeTab key={current.id} record={current} />}
        {tab === "deductions" && (
          <DeductionsTab key={current.id} record={current} />
        )}
        {tab === "simulator" && (
          <SimulatorTab key={current.id} record={current} />
        )}
      </div>
    </div>
  );
...
function SummaryTab({
  ...
  year: number;
  onSwitch: (t: "summary" | "income" | "deductions" | "simulator") => void;
}) {
  ...
  return (
    <div className="px-4 py-4 space-y-3">
      {/* Hero */}
      <div
        className={`rounded-2xl p-5 text-white ${breakdown.netTaxPayable > 0 ? "bg-gradient-to-br from-orange-500 to-red-600" : "bg-gradient-to-br from-green-500 to-emerald-600"}`}
      >
        <div className="text-xs opacity-75 mb-1">
          {breakdown.netTaxPayable >= 0
            ? "ภาษีที่ต้องจ่ายเพิ่ม"
            : "ภาษีที่ขอคืนได้"}
        </div>
        <div className="text-3xl font-bold">
          {formatCurrency(Math.abs(breakdown.netTaxPayable), 0)}
        </div>
        <div className="text-xs opacity-75 mt-1">
          ภาษีรวม {formatCurrency(breakdown.taxOwed, 0)} − หัก ณ ที่จ่าย{" "}
          {formatCurrency(breakdown.withholding, 0)}
        </div>
        <div className="grid grid-cols-3 gap-2 mt-3">
          <div className="bg-white/15 rounded-xl p-2 text-center">
            <div className="text-[10px] opacity-75">รายได้รวม</div>
            <div className="text-[13px] font-bold">
              {formatCurrency(breakdown.grossIncome, 0)}
            </div>
          </div>
          <div className="bg-white/15 rounded-xl p-2 text-center">
            <div className="text-[10px] opacity-75">เงินได้สุทธิ</div>
            <div className="text-[13px] font-bold">
              {formatCurrency(breakdown.netIncome, 0)}
            </div>
          </div>
          <div className="bg-white/15 rounded-xl p-2 text-center">
            <div className="text-[10px] opacity-75">Marginal Rate</div>
            <div className="text-[13px] font-bold">
              {(breakdown.marginal * 100).toFixed(0)}%
            </div>
          </div>
        </div>
      </div>
      ...
      {suggestions.length > 0 && (
        <Card>
          ...
          {suggestions.slice(0, 5).map((s) => {
            ...
            const sim = simulateTaxSaving(
              record,
              s.field as keyof TaxRecord,
              s.unused,
            );
            return (
              <div
                key={s.name}
                className="flex justify-between items-center py-1.5 text-[13px] border-b border-gray-50 last:border-0"
              >
                <div>
                  <div className="font-semibold text-gray-700">{s.name}</div>
                  <div className="text-[11px] text-gray-400">
                    เหลือใช้ {formatCurrency(s.unused, 0)} (เพดาน{" "}
                    {formatCurrency(s.cap, 0)})
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-[12px] font-bold text-green-600">
                    ประหยัด {formatCurrency(sim.saving, 0)}
                  </div>
                </div>
              </div>
            );
      ...
      <Card>
        ...
        <div className="space-y-1.5 text-[13px] mt-2">
          ...
          <Row
            label="หัก ค่าใช้จ่าย 50% (เพดาน 100k)"
            value={-breakdown.expenseAllowance}
            negative
          />
          <Row
            label="หัก ค่าลดหย่อน"
            value={-breakdown.totalDeductions}
            negative
          />
          <div className="border-t pt-1.5 mt-1.5">
            <Row
              label="เงินได้สุทธิ (ฐานภาษี)"
          ...
          <Row
            label="หัก ณ ที่จ่ายแล้ว"
            value={-breakdown.withholding}
            negative
          />
          <div className="border-t pt-1.5 mt-1.5">
            <Row
              ...
              bold
              color={
                breakdown.netTaxPayable >= 0 ? "text-red-500" : "text-green-600"
              }
            />
          </div>
          <div className="text-[11px] text-gray-400 mt-2">
            อัตราเฉลี่ย {(breakdown.effective * 100).toFixed(2)}% · Marginal{" "}
            {(breakdown.marginal * 100).toFixed(0)}%
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="mt-2 text-[12px] space-y-0.5">
          {TAX_BRACKETS.map((b, i) => {
            const isCurrent =
              breakdown.netIncome > b.min && breakdown.netIncome <= b.max;
            return (
              <div
                key={i}
                className={`flex justify-between py-1 ${isCurrent ? "bg-indigo-50 -mx-2 px-2 rounded font-bold text-indigo-700" : ""}`}
              >
                <span>
...
function initForm(record: TaxRecord, fields: (keyof TaxRecord)[]): FormState {
  ...
  for (const k of fields)
    f[k] = (record[k] as number | undefined)?.toString() ?? "";
  return f;
}
...
function Field({
  ...
  return (
    <div className="mb-2.5">
      <div className="text-[12px] font-semibold text-gray-600 mb-0.5">
        {label}
      </div>
...
// ── Income Tab ─────────────────────────────────────────────────────────────
const INCOME_FIELDS: (keyof TaxRecord)[] = [
  "totalIncome",
  "bonus",
  "otherIncome",
  "withholdingTax",
...
function IncomeTab({ record }: { record: TaxRecord }) {
  ...
  const set = (k: keyof TaxRecord) => (v: string) => {
    setForm((f) => ({ ...f, [k]: v }));
    setSaved(false);
  };
    ...
    <div className="px-4 py-4 space-y-3 pb-8">
      <Card>
        ...
        <Field
          ...
          help="40(1) เงินได้จากการจ้างงาน"
          value={form.totalIncome ?? ""}
          onChange={set("totalIncome")}
        />
        <Field
          label="โบนัส"
          help="โบนัสประจำปี / เงินพิเศษ"
          value={form.bonus ?? ""}
          onChange={set("bonus")}
        />
        <Field
          label="รายได้อื่น"
          help="Freelance, ปันผล, ดอกเบี้ย"
          value={form.otherIncome ?? ""}
          onChange={set("otherIncome")}
        />
        <Field
          label="ภาษีหัก ณ ที่จ่าย"
          help="ที่ถูกหักไปแล้วจากเงินเดือน"
          value={form.withholdingTax ?? ""}
          onChange={set("withholdingTax")}
        />
      </Card>
...
const INT_FIELDS: (keyof TaxRecord)[] = ["childrenCount", "childrenAfter2561"];

function DeductionsTab({ record }: { record: TaxRecord }) {
  const [form, setForm] = useState(() => {
    const initial = initForm(record, DEDUCTION_FIELDS);
    if (
      (initial.parentsSupport === "" || initial.parentsSupport === undefined) &&
      (record.parentsCount || 0) > 0
    ) {
      const legacy = Math.min(record.parentsCount || 0, 4) * 30_000;
      initial.parentsSupport = legacy.toString();
    }
    return initial;
  });
  const [saved, setSaved] = useState(false);
  ...
  return (
    <div className="px-4 py-4 space-y-3 pb-8">
      <Card>
        ...
        <Field
          label="คู่สมรส (60,000)"
          help="0 หรือ 60,000"
          value={form.spouseAllowance ?? ""}
          onChange={set("spouseAllowance")}
        />
        <Field
          label="จำนวนบุตร"
          help="คนแรก 30k, คนที่ 2+ เกิดก่อน 2561 30k"
        ...
        />
        <Field
          label="บุตร (เกิดตั้งแต่ 2561, ตั้งแต่คนที่ 2)"
          help="60,000/คน"
          value={form.childrenAfter2561 ?? ""}
          onChange={set("childrenAfter2561")}
          min={0}
          step={1}
        />
        <Field
          label="อุปการะบิดามารดา"
          help="ใส่ยอดลดหย่อนที่ใช้สิทธิ์จริง (≤ 180,000)"
          value={form.parentsSupport ?? ""}
          onChange={set("parentsSupport")}
          min={0}
          max={180_000}
          step={100}
        />
      ...
      <Card>
        ...
        <Field
          label="ประกันชีวิต"
          help="≤ 100,000 (รวมประกันสุขภาพ)"
          value={form.lifeInsurance ?? ""}
          onChange={set("lifeInsurance")}
        />
        <Field
          label="ประกันสุขภาพตน"
          help="≤ 25,000 (รวมประกันชีวิต ≤ 100,000)"
          value={form.healthInsurance ?? ""}
          onChange={set("healthInsurance")}
        />
        <Field
          label="ประกันสุขภาพบิดามารดา"
          help="≤ 15,000"
          value={form.parentsHealthInsurance ?? ""}
          onChange={set("parentsHealthInsurance")}
        />
        <Field
          label="ประกันชีวิตแบบบำนาญ"
          help="≤ 200,000 หรือ 15% รายได้"
          value={form.pensionInsurance ?? ""}
          onChange={set("pensionInsurance")}
        />
        <Field
          label="ประกันสังคม"
          help="≤ 9,000 (750 × 12)"
      ...
      <Card>
        ...
        <div className="text-[11px] text-amber-600 mt-1 mb-2">
          ⚠️ PVD + บำนาญ + RMF + SSF รวมไม่เกิน 500,000
        </div>
        <Field
          label="PVD (พนักงานจ่าย)"
          help="≤ 500k หรือ 15% รายได้"
          value={form.pvdContribution ?? ""}
          onChange={set("pvdContribution")}
        />
        <Field
          label="RMF"
          help="≤ 500k หรือ 30% รายได้"
          value={form.rmf ?? ""}
          onChange={set("rmf")}
        />
        <Field
          label="SSF"
          help="≤ 200k หรือ 30% รายได้"
          value={form.ssf ?? ""}
          onChange={set("ssf")}
        />
        <Field
          label="Thai ESG Fund"
          help="≤ 300k หรือ 30% รายได้ (cap แยก)"
          value={form.thaiEsg ?? ""}
          onChange={set("thaiEsg")}
        />
      </Card>
      ...
      <Card>
        ...
        <Field
          ...
          help="≤ 50,000 (ปี 2567)"
          value={form.easyEReceipt ?? ""}
          onChange={set("easyEReceipt")}
        />
        <Field
          label="เงินบริจาคทั่วไป"
          help="≤ 10% ของรายได้หลังลดหย่อน"
          value={form.donation ?? ""}
          onChange={set("donation")}
        />
        <Field
          label="บริจาคการศึกษา/สาธารณสุข (×2)"
          help="คูณ 2, ≤ 10% หลังลดหย่อน"
          value={form.donationEducation ?? ""}
          onChange={set("donationEducation")}
        />
        <Field
          label="บริจาคพรรคการเมือง"
          help="≤ 10,000"
          value={form.donationPolitical ?? ""}
          onChange={set("donationPolitical")}
        />
      </Card>
    ...
    <div className="px-4 py-4 space-y-3 pb-8">
      <Card className="!bg-gradient-to-br from-indigo-600 to-purple-700 !text-white">
        ...
        <div className="text-lg font-bold mb-3">
          {formatCurrency(baseTax, 0)}
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-white/15 rounded-xl p-2.5">
            <div className="text-[11px] opacity-75">ภาษีใหม่</div>
            <div className="text-[16px] font-bold">
              {formatCurrency(newTax, 0)}
            </div>
          </div>
          <div className="bg-white/15 rounded-xl p-2.5">
            <div className="text-[11px] opacity-75">ประหยัดได้</div>
            <div className="text-[16px] font-bold text-green-300">
              {formatCurrency(saving, 0)}
            </div>
          </div>
        </div>
        {totalInvest > 0 && (
          <div className="mt-3 text-[12px] opacity-90">
            ลงทุน {formatCurrency(totalInvest, 0)} → ประหยัดภาษี{" "}
            <b className="text-green-300">{roi.toFixed(1)}%</b> ของยอดที่ลง
          </div>
        )}
      </Card>
