...
export function calcThaiTax(t: TaxRecord): TaxBreakdown {
  ...
  d['อุปการะบิดามารดา'] = Math.min(t.parentsCount || 0, 4) * 30_000
  ...
  d['ประกันชีวิตแบบบำนาญ'] = Math.min(t.pensionInsurance || 0, gross * 0.15, 200_000)
  ...
  const rmf = Math.min(t.rmf || 0, gross * 0.30, 500_000)
  const ssf = Math.min(t.ssf || 0, gross * 0.30, 200_000)
  // ตรวจ cap รวม 500k (PVD + บำนาญ + RMF + SSF) -- ยกเว้น ESG, Easy E-Receipt
  const pensionCombo = pvd + (d['ประกันชีวิตแบบบำนาญ'] || 0) + rmf + ssf
  let pvdFinal = pvd, rmfFinal = rmf, ssfFinal = ssf, pensionFinal = d['ประกันชีวิตแบบบำนาญ']
  if (pensionCombo > 500_000) {
    ...
    const order = [
      ['ssf', () => ssfFinal, (v: number) => { ssfFinal = v }],
      ['rmf', () => rmfFinal, (v: number) => { rmfFinal = v }],
      ['pension', () => pensionFinal, (v: number) => { pensionFinal = v }],
    ] as const
  ...
  d['Thai ESG Fund'] = Math.min(t.thaiEsg || 0, gross * 0.30, 300_000)
  ...
  const donationCap = beforeDonation * 0.10
...
export function simulateTaxSaving(t: TaxRecord, field: keyof TaxRecord, additionalAmount: number): { newTax: number; saving: number } {
  const baseTax = calcThaiTax(t).taxOwed
  const updated: TaxRecord = { ...t, [field]: ((t[field] as number) || 0) + additionalAmount }
...
export function suggestUnusedAllowances(t: TaxRecord): { name: string; field: keyof TaxRecord; unused: number; cap: number }[] {
  const gross = (t.totalIncome || 0) + (t.bonus || 0) + (t.otherIncome || 0)
  const items: { name: string; field: keyof TaxRecord; unused: number; cap: number }[] = []
  // RMF
  const rmfCap = Math.min(gross * 0.30, 500_000)
  items.push({ name: 'RMF', field: 'rmf', cap: rmfCap, unused: Math.max(rmfCap - (t.rmf || 0), 0) })
  // SSF
  const ssfCap = Math.min(gross * 0.30, 200_000)
  items.push({ name: 'SSF', field: 'ssf', cap: ssfCap, unused: Math.max(ssfCap - (t.ssf || 0), 0) })
  // Thai ESG
  const esgCap = Math.min(gross * 0.30, 300_000)
  items.push({ name: 'Thai ESG', field: 'thaiEsg', cap: esgCap, unused: Math.max(esgCap - (t.thaiEsg || 0), 0) })
  // Life ins (combined with health)
  const lifeUsed = (t.lifeInsurance || 0) + (t.healthInsurance || 0)
  items.push({ name: 'ประกันชีวิต/สุขภาพ', field: 'lifeInsurance', cap: 100_000, unused: Math.max(100_000 - lifeUsed, 0) })
  // Pension ins
  const penCap = Math.min(gross * 0.15, 200_000)
  items.push({ name: 'ประกันบำนาญ', field: 'pensionInsurance', cap: penCap, unused: Math.max(penCap - (t.pensionInsurance || 0), 0) })
  return items.filter(i => i.unused > 0).sort((a, b) => b.unused - a.unused)
}

export function emptyTaxRecord(year: number): Omit<TaxRecord, 'id'> {
  return {
    year, totalIncome: 0, bonus: 0, otherIncome: 0,
    personalAllowance: 60_000, spouseAllowance: 0,
    childrenCount: 0, childrenAfter2561: 0, parentsCount: 0,
    lifeInsurance: 0, healthInsurance: 0, parentsHealthInsurance: 0,
    pensionInsurance: 0, socialSecurity: 9_000,
    pvdContribution: 0, rmf: 0, ssf: 0, thaiEsg: 0,
    mortgageInterest: 0, donation: 0, donationEducation: 0, donationPolitical: 0,
    easyEReceipt: 0, withholdingTax: 0,
